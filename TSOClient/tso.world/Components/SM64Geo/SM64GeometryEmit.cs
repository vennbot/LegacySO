...
namespace FSO.LotView.Components.SM64Geo
{
    ...
    internal class SM64GeometryEmit : AbstractGeometryEmit, IDisposable
    {
        private Matrix[] MatStack = new Matrix[32];
        private int MatStackPos;
        private Light Ambient;
        private Light Directional;
        private Vector2 ActiveColorUV;

        private int VertOffset = 0;
        private List<VitaboyVertex> AllVertices = new List<VitaboyVertex>();
        private List<int> Indices = new List<int>();

        // Deferred mode only
        private Matrix ActiveMatrix;
        private List<Matrix> Matrices = new List<Matrix>();
        private List<List<VitaboyVertex>> VerticesPerMatrix = new List<List<VitaboyVertex>>();
        ...
        private Dictionary<Light, Vector2> ColorToUV = new Dictionary<Light, Vector2>()
        {
            { new Light(0, 0, 255), new Vector2(1/6f, 1/4f) }, // Blue
            { new Light(255, 0, 0), new Vector2(3/6f, 1/4f) }, // Red
            { new Light(254, 193, 121), new Vector2(5/6f, 1/4f) }, // Don't know really
            { new Light(115, 6, 0), new Vector2(1/6f, 3/4f) }, // Brown?
            { new Light(255, 255, 255), new Vector2(3/6f, 3/4f) }, // White
            { new Light(114, 28, 14), new Vector2(5/6f, 3/4f) }, // IDK
        };
            ...
            if (DeferredMode)
            {
                if (!FirstList)
                {
                    if (drawing)
                    {
                        ...
                        AllVertices = new List<VitaboyVertex>();
                    }
        ...
        public override void Light(Light l, int n)
        {
            ...
            {
                Directional = l;
                if (!ColorToUV.TryGetValue(l, out ActiveColorUV))
                {
        ...
        private Vector3 ToNormal(Cn colorNormal)
        {
            // From the active lighting factor, and the normal applied to the model
            return new Vector3((sbyte)colorNormal.R, (sbyte)colorNormal.G, (sbyte)colorNormal.B) / 0x7F;
        }
        ...
        public override void Vertices(Vtx[] vertices, int n, int v0)
        {
            ...
            if (DeferredMode)
            {
                for (int i = v0; i < end; i++)
                {
                    ...
                    var pos = ToVector3(vtx.Pos);
                    var normal = ToNormal(vtx.Cn);
                    AllVertices.Add(new VitaboyVertex(
                        pos,
                        ActiveColorUV,
                        pos,
                        new Vector3(mtxN, mtxN, 0),
                        normal,
                        normal));
                }
            }
            else
            {
                ...
                {
                    ref Vtx vtx = ref vertices[i];
                    AllVertices.Add(new VitaboyVertex(Vector3.Transform(ToVector3(vtx.Pos), mat), ToColor(vtx.Cn, ref mat)));
                }
                */
            }
        }
        ...
        public Tuple<VertexBuffer, IndexBuffer, int> Complete(GraphicsDevice gd)
        {
            if (GDVertices == null)
            {
                ...
                GDVertices = new VertexBuffer(gd, typeof(VitaboyVertex), vertCount, BufferUsage.None);
