...
namespace FSO.Patcher
{
    public partial class Patcher : Form
    {
        ...
        private void Patcher_Load(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Could not find LegacySO Patch Files (these must be downloaded by the game!). Starting LegacySO...");
                StartFreeSO();
                return;
            }
        ...
        {
            StatusLabel.Text = "Extracting LegacySO Files...";
            ...
            foreach (var entry in entries)
            {
                ...
                while (true)
                {
                    ...
                    if (!result)
                    {
                        var dresult = MessageBox.Show("Couldn't replace a file. Make sure you are not running an instance of LegacySO! If this is discord-rpc.dll, you can safely ignore this.", "Error", MessageBoxButtons.AbortRetryIgnore);
        ...
        public void AttemptRename()
        {
            try
            {
                File.Delete("LegacySO.exe.old");
                if (File.Exists("LegacySO.exe"))  //shouldn't be in use, unless the user has incorrectly renamed and run the freeso executable
                    File.Move("LegacySO.exe", "LegacySO.exe.old");
            }
            catch (Exception)
            {
                if (RenameRetry++ < RENAME_MAX_ATTEMPTS)
                {
                    StatusLabel.Text = "Waiting for LegacySO to Close...";
                    Task.Run(async () =>
                    {
                ...
                {
                    var result = MessageBox.Show("Could not update LegacySO as write access could not be gained to the game files. Try running update.exe as an administrator.", "Error", MessageBoxButtons.RetryCancel);
        ...
        public void Cleanup()
        {
            try
            {
                if (File.Exists("LegacySO.exe.old"))
                    File.Move("LegacySO.exe.old", "LegacySO.exe");
            }
            catch (Exception)
            {
        ...
        public void StartFreeSO()
        {
            ...
            {
                System.Diagnostics.Process.Start("mono", "LegacySO.exe " + string.Join(" ", Args));
            }
            else
            {
                System.Diagnostics.Process.Start("LegacySO.exe", string.Join(" ", Args));
            }
            Application.Exit();
        }
