...
namespace FSO.Patcher
{
    public class CLIPatcher
    {
        ...
        {
            Console.WriteLine("Could not update LegacySO as write access could not be gained to the game files. Try running update.exe as an administrator.");
            Cleanup();
            Environment.Exit(0);
        }
        ...
        private async Task<int> ShowErrors(List<string> remaining)
        {
            ...
            Console.WriteLine("Couldn't write one or more files. Make sure you are not running an instance of LegacySO! \r\nFiles:\r\n\r\n" + fileList);
            return 0;
        }
        ...
        {
            if (!File.Exists("LegacySO.exe")) File.Copy("LegacySO.exe.old", "LegacySO.exe", true);
            if (Environment.OSVersion.Platform == PlatformID.Unix || Environment.OSVersion.Platform == PlatformID.MacOSX)
            {
                Console.WriteLine($"===== Starting LegacySO... Please wait! =====");
                var args = string.Join(" ", Args);
                if (args.Length > 0) args = " " + args;
                var startArgs = new ProcessStartInfo("mono", "LegacySO.exe" + args);
        ...
        public void Begin()
        {
            Console.WriteLine("===== LegacySO Patcher CLI - 2019 =====");
            Console.WriteLine(Path.Count + " update(s) to apply.");

            if (Args.Contains("--client"))
            {
                Console.WriteLine("LegacySO client requested. Downloading from servo.legacyso.org.");
                ToDownload.Add("https://fso-builds.riperiperi.workers.dev/"); //BV-TO-REPAIR
            }

            if (Args.Contains("--extras"))
            {
                Console.WriteLine("Unix Extras requested. Downloading from LegacySO.org.");
                ToDownload.Add("http://freeso.org/stuff/macextras.zip"); //BV-TO-REPAIR
