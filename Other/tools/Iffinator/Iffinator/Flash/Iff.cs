...
namespace Iffinator.Flash
{
    public class Iff
    {
        ...
        /// <summary>
        /// Constructor used by Iffinator to scan through the files of the game.
        /// This constructor will NOT read chunkdata, and as such the resulting 
        /// instance will only contain a list of Chunk instances.
        /// </summary>
        /// <param name="Path">The path of an IFF archive to read from.</param>
        /// <param name="IsIff">Is this an IFF or SPF archive?</param>
        public Iff(byte[] Data, string Filename)
        {
            m_Reader = new BinaryReader(new MemoryStream(Data), Encoding.ASCII);

            Dictionary<string, List<uint>> Files = new Dictionary<string, List<uint>>();
            //Don't bother looking for the RSMP, just loop through the file.
            FuckThisShit(ref Files);

            foreach (KeyValuePair<string, List<uint>> KVP in Files)
            {
                foreach (uint Offset in KVP.Value)
                {
